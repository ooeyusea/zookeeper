...
DataSet::DataSet(IStateDataFactory * factory)
	: _mutex(false)
	, _factory(factory) {
	_stateData = _factory->Create();
}
...
bool DataSet::CheckOrDrop(const std::string& data) {
	hyper_net::shared_lock_guard<hn_shared_mutex> guard(_mutex);
	if (!_uncommit.empty()) {
		throw std::logic_error("invalid zxId");
	}

	if (_stateData->PreCheck(data))
		return true;
...
void DataSet::Commit(int64_t zxId) {
	...
	if (_uncommit.empty() || _uncommit.begin()->zxId != zxId) {
		...
		_stateData->Apply(_commitLog.back().data);
		_zxId = zxId;
...
void DataSet::Read(std::string&& data, std::string& result) {
	hn_shared_lock_guard<hn_shared_mutex> guard(_mutex);
	result = _stateData->Read(data);
}
...
#ifdef WIN32
			char * data = (char*)alloca(_logFileheader.logs[i].size);
#else
			char data[header.logs[i].size];
#endif
			if (fread(data, _logFileheader.logs[i].size, 1, _lastLogFile) < 1) {
				hn_error("log data file {} corrupted", fileId);
				return false;
			}

			_commitLog.push_back({ _zxId, std::string(data, _logFileheader.logs[i].size) });
			_stateData->Apply(_commitLog.rbegin()->data);
