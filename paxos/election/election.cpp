...
namespace paxos {
	...
	void VoteSender::Start(std::string ip, int32_t port) {
		while (!_terminate) {
			...
			_activeTick = olib::GetTickCount();

			auto test = olib::DoWork([this] { CheckTimeout(); });
			auto pong = olib::DoWork([this] { DealPingpong(); });
	...
	void VoteReciver::Start(int32_t fd, hn_channel<Vote, -1> ch) {
		...
		auto test = olib::DoWork([this, fd] { CheckTimeout(fd); });
		auto tsp = olib::DoWork([this, fd] { TickSendPing(fd); });
		auto np = olib::DoWork([this, fd, ch] { DealNetPacket(fd, ch); });
	...
	Vote Election::LookForLeader(int32_t id, int64_t zxId, int32_t count) {
		...
		int64_t mark = olib::GetTickCount();
		std::list<Vote> queue;
		while (true) {
			Vote vote;
			if (!queue.empty()) {
			...
			case LOOKING:
				...
				if (IsVoteOk(votes, vote.voteId, vote.electionEpoch, count)) {
					int64_t check = olib::GetTickCount();
					while (true) {
						if (olib::GetTickCount() - check > WAIT_TIME)
							break;
